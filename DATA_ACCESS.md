...
| **cores_v1** and **sample_v2.parquet** (derived from the dump) | The analysis file: 153,012 partner-attributed rows, 2010-01 to 2023-10, companies with no acquisition or IPO on or before the round date | **No** — derived from licensed microdata (row-level identifiers) | Rebuild with `code/pipeline/build_cores.py` then `build_sample_v2.py`; the SHA-256 prefix and row count to match are in `artifacts/sample_manifest.yaml` |
...
`code/pipeline/` is the complete analysis chain: `build_cores.py` → `build_sample_v2.py` → the pre-study harness `i73 … i82` → the paper's runs `p001_00 … p001_63` (`run_all_v2.py`). With the Crunchbase export in place and the environment variables set (`README.md`), each script writes its JSON artifact into `P001_OUT`; `code/build/p001_09_exhibits.py` → `p001_09d_compose_v9.py` → `p001_09b_assemble_tables.py` then regenerate the exhibits from those artifacts, and `notebooks/01_tables.ipynb` compares them with the paper's.
