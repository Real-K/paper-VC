# -*- coding: utf-8 -*-
"""p001_21b 승계 검정 정정판 — 대조군을 "여성 파트너 보유 회사"로 제한

[정정 사유] 원판(P001-21)의 여성 대체 채용 DiD +14.2pp 는 구성상 교란 가능: 여성 이탈 사건은
정의상 "여성 파트너가 있던 회사"에서만 발생하고, 그런 회사는 남성 이탈 회사보다 애초에 여성을
더 뽑는다. 대조군(남성 이탈)을 **사전 창에 여성 귀속 파트너가 1명 이상인 회사**로 제한하고
사전 여성 비중 3분위로 재가중해야 "대체"와 "회사 유형"이 분리된다. 원판도 원장 보존.

(원 docstring) p001_21 승계 검정 — 여성 파트너가 떠난 뒤 그 딜 채널을 누가 이어받는가
...
for e in dep.itertuples():
    ...
    pre_partners = set(pre["partner_uuid"]) - {e.partner_uuid}
    n_fem_pre = sum(1 for p in pre_partners if g_map.get(p) == "female")
    pre_has_fem = 1.0 if n_fem_pre >= 1 else 0.0
    pre_fem_share = (n_fem_pre / len(pre_partners)) if pre_partners else 0.0
    rows.append({
        ...
        "pre_has_fem": pre_has_fem, "pre_fem_share": pre_fem_share,
        "n_post_new": int(len(post_new))})
E = pd.DataFrame(rows)
# 정정: 대조군(남성 이탈)을 사전 창에 여성 파트너가 있던 회사로 제한 + 사전 여성 비중 3분위 재가중
E = E[(E["fem"] == 1.0) | (E["pre_has_fem"] == 1.0)].reset_index(drop=True)
if len(E) > 60:
    E["fq"] = pd.qcut(E["pre_fem_share"].rank(method="first"), 3, labels=False, duplicates="drop").astype(str)


def did(col, nb=NB):
    sub = E[["fem", "firm", "fq", col]].dropna().reset_index(drop=True)
    # 사전 여성 비중 3분위 셀 내 차분 (셀 demean)
    sub[col] = sub[col] - sub.groupby("fq")[col].transform("mean")
...
emit("P001-21b", "승계 검정 — 이탈 후 채널을 누가 이어받는가 (§5 메커니즘)", status,
     ...
     extra={"stage": 6, "feeds": "§5 비대칭의 메커니즘 (논문 최대 novelty 후보)", "slug": "succession_fix"})
print("done")
