# -*- coding: utf-8 -*-
"""p001_57 — [v2.1 2026-09-10: exit5·exit8 를 병합 전에 구성(인덱스 재정렬 버그 수정; 지평 기저율 단조성·행수 동일성 가드) · v2: exit5·exit8 추가, 결과변수 간 짝지은 추첨으로 지평 차이 구간(c1 §5·c2 §4), 파트너 FE+시장 셀 FE 동시 사양(c1 §6·c2 §4); sample_v2]
P001-54 후속: 파트너 내 결과 검정의 결과 구성물 분해 — 같은 창(딜 ≤2017-10)에서 exit3 · exit6 · exit_ever · fon, 그리고 창(≤2017-10 vs ≤2020-10) 의 분리
...
from p001_rescue_common import COMMON_SHA, CTX, CUT, END, END_FON, boot, build, emit, load_deals, log, qci
...
dn["exit5"] = (days <= 365 * 5).fillna(False).astype(float)   # v2: 60-month horizon — built here, before the merges below re-index the frame
dn["exit8"] = (days <= 365 * 8).fillna(False).astype(float)   # v2: 96-month horizon (deals through 2017-10 close by 2025-10, inside the data)
...
def run(s, tag, key):
    """v2: 같은 시드로 부트를 시작해 결과변수 간 추첨을 짝지음(같은 행·같은 파트너 군집 순서) → 지평 간 차이의 구간을 같은 추첨에서 계산."""
    global rng
    rng = np.random.default_rng(SEED)
    res = boot(s, "r", ["ff", "ffp"] + COVS, ["ff", "ffp"], rng, nb=NB, demean="partner_uuid", cluster="partner_uuid", min_n=500, return_draws=True)
    DR[key] = res["_draws"][:, res["_xc"].index("ffp")]; res = {k: v for k, v in res.items() if not k.startswith("_")}
    sd = float(s["r"].std())
    for k in ("ff", "ffp"):
    ...
    log(f"  {tag:<34} β_int {res['ffp']['coef']*100:+.2f}pp [{res['ffp']['ci95'][0]*100:+.2f},{res['ffp']['ci95'][1]*100:+.2f}] MDE {res['ffp']['mde80']*100:.2f} ({res['ffp']['beta_std']:+.3f} sd) · n {res['n']:,}")
    return res


def twoway(s, y, tag, nb=NB, sweeps=12):
    """v2 (c1 §6 · c2 §4): 원 결과 y 에 파트너 FE 와 시장 셀(연×섹터×단계) FE 를 동시에 넣은 사양 — 벤치마크 차감(r) 사양과 비교. 파트너 군집 부트."""
    def fit_tw(df):
        cols = [y, "ff", "ffp"] + COVS; M = df[cols].to_numpy(float).copy(); gp = df["partner_uuid"].to_numpy(); gc = df["yss"].to_numpy()
        for _ in range(sweeps):
            for g in (gp, gc):
                mm = pd.DataFrame(M).groupby(g).transform("mean").to_numpy(); M = M - mm
        X = np.column_stack([M[:, 1:], np.ones(len(M))]); b = np.linalg.lstsq(X, M[:, 0], rcond=None)[0]; return b[:2]
    cnt = s.groupby("partner_uuid")[y].transform("size"); dd = s[cnt >= 2].reset_index(drop=True)
    b0 = fit_tw(dd); grp = {c: g.index.to_numpy() for c, g in dd.groupby("partner_uuid")}; keys = list(grp); bs = []
    r2 = np.random.default_rng(SEED + 1)
    for _ in range(nb):
        pick = r2.integers(0, len(keys), len(keys)); sub = dd.loc[np.concatenate([grp[keys[i]] for i in pick])].reset_index(drop=True)
        try: bs.append(fit_tw(sub))
        except Exception: pass
    bs = np.array(bs); out = {}
    for i, k in enumerate(("ff", "ffp")):
        lo, hi = qci(bs[:, i]); se = float(np.std(bs[:, i], ddof=1)); out[k] = {"coef": round(float(b0[i]), 5), "ci95": [round(lo, 5), round(hi, 5)], "sig": bool(lo > 0 or hi < 0), "se_boot": round(se, 5), "mde80": round(2.8 * se, 4)}
    out.update({"n": int(len(dd)), "n_partners": int(dd["partner_uuid"].nunique()), "spec": "y ~ ff + ffp + X | partner FE + year×sector×stage FE (iterative demeaning)"})
    log(f"  {tag:<34} β_int {out['ffp']['coef']*100:+.2f}pp [{out['ffp']['ci95'][0]*100:+.2f},{out['ffp']['ci95'][1]*100:+.2f}] MDE {out['ffp']['mde80']*100:.2f} · β_FF {out['ff']['coef']*100:+.2f} · n {out['n']:,}")
    return out


def hdiff(a, b):
    d = DR[a] - DR[b]; lo, hi = qci(d)
    return {"coef": round(float(OUT["A_2017"][a]["ffp"]["coef"] - OUT["A_2017"][b]["ffp"]["coef"]), 5), "ci95": [round(lo, 5), round(hi, 5)], "sig": bool(lo > 0 or hi < 0), "paired_draws": int(len(d))}


log("\n" + "=" * 100 + "\n[A] 창 ≤2017-10 — 구성물·지평 사다리 (36·60·72·96개월·eventual·36개월 이후·후속), 짝지은 추첨\n" + "=" * 100)
OUT["A_2017"] = {}
for y in ("exit3", "exit5", "exit6", "exit8", "exit_ever", "late_exit", "fon"):
    s = sample(y, CUT); OUT["A_2017"][y] = run(s, f"A ≤2017-10 y={y}", y); OUT["A_2017"][y]["base_y"] = round(float(s[y].mean()), 4)
OUT["A_2017"]["late_exit"]["definition"] = "indicator: exit_ever = 1 and exit3 = 0, on the full sample (not conditional on surviving to month 36); equals exit_ever minus exit3 by construction"
_b = [OUT["A_2017"][k]["base_y"] for k in ("exit3", "exit5", "exit6", "exit8", "exit_ever")]; _n = {OUT["A_2017"][k]["n"] for k in ("exit3", "exit5", "exit6", "exit8", "exit_ever")}
assert all(_b[i] <= _b[i + 1] + 1e-12 for i in range(4)) and len(_n) == 1, ("horizon indicators do not nest / row counts differ", _b, _n)   # v2.1 guard (2026-09-10: caught a re-index misalignment of exit5/exit8)
OUT["A_horizon_differences_same_draws"] = {f"{h}_minus_exit3": hdiff(h, "exit3") for h in ("exit5", "exit6", "exit8", "exit_ever")}
OUT["A_horizon_differences_same_draws"]["exit_ever_minus_exit8"] = hdiff("exit_ever", "exit8")
for k, v in OUT["A_horizon_differences_same_draws"].items(): log(f"  Δ {k:<22} {v['coef']*100:+.2f}pp [{v['ci95'][0]*100:+.2f},{v['ci95'][1]*100:+.2f}]")
log("\n" + "=" * 100 + "\n[B] 창 ≤2020-10 — exit3 · fon (P001-54 재현) · [C] 파트너 FE + 시장 셀 FE 동시 사양\n" + "=" * 100)
OUT["B_2020"] = {}
for y in ("exit3", "fon"):
    s = sample(y, END_FON); OUT["B_2020"][y] = run(s, f"B ≤2020-10 y={y}", "B_" + y); OUT["B_2020"][y]["base_y"] = round(float(s[y].mean()), 4)
OUT["C_twoway_FE"] = {"exit3_2020": twoway(sample("exit3", END_FON), "exit3", "C two-way FE exit3 ≤2020"), "exit_ever_2017": twoway(sample("exit_ever", CUT), "exit_ever", "C two-way FE exit_ever ≤2017")}
...
pred = {"A_exit3_in_[-3,2]_incl0": -0.03 <= a["exit3"]["ffp"]["coef"] <= 0.02 and not a["exit3"]["ffp"]["sig"], "A_exit6_in_[-5,1]": -0.05 <= a["exit6"]["ffp"]["coef"] <= 0.01,
        ...
        "B_exit3_reproduces": abs(b["exit3"]["ffp"]["coef"] - 0.0048) < 0.0005, "A_partners_1998": a["exit_ever"]["n_partners"] == 1998, "A_ev_minus_e3_negative": OUT["A_horizon_differences_same_draws"]["exit_ever_minus_exit3"]["coef"] < 0}
...
emit("P001-57", "P001-54 후속: 파트너 내 결과 검정의 지평 사다리 (같은 창·같은 추첨: 36·60·72·96개월·eventual·36개월 이후·후속) + 파트너 FE×시장 셀 FE 동시 사양", status, OUT,
