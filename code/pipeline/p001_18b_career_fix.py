# -*- coding: utf-8 -*-
"""p001_18b (정정판) 커리어 검정 — 스타일 통제 추가

[정정 사유] 원판(P001-18): raw −1.60 | adj +1.43 — 미래 딜 "건수"가 스테이지 스타일에 오염
(조정 고정 시 raw 高 = 후기 셀 = 원래 건수 적음). 파트너의 과거 초기단계 비중·섹터 폭을
통제해 스타일 채널 분리. 원판도 원장 보존.

(원 docstring) 커리어 검정 — 시장은 원시 트랙레코드를 가격에 반영하는가 (finance 심판 #3)
...
hist["early"] = hist["stage"].isin({"seed","angel","pre_seed","equity_crowdfunding","convertible_note"}).astype(float)
...
emit("P001-18b", "커리어 검정 — 원시 vs 조정 랭크의 미래 딜플로우 가격 반영 (Track C-⑦)", status,
     ...
     extra={"stage": 5, "feeds": "finance#3 / §6.1·Table 7 격상 판정", "slug": "career_stylectrl"})
print("done")
